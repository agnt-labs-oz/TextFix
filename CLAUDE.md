...
```bash
dotnet test                                              # all 255 tests
dotnet test --filter FullyQualifiedName~AppSettingsTests  # single test class
```

255 cases. Note that xUnit expands every `[Theory]`/`[InlineData]` pair into its own case, so counting attributes in the source undercounts — trust `dotnet test`.
